...
def health_cost_for_age(age: int) -> float:
    ...
    ranges = PREMISES["saude"]["gastoAnualPorFaixa"]
    last_cost = ranges[-1]["gasto"]
    for entry in ranges:
        faixa = entry["faixa"]
        gasto = entry["gasto"]
        if "–" in faixa:
...
def compute_costs_and_incomes(
    ...
    bairros = {b["nome"]: b["precoM2"] for b in PREMISES["moradia"]["bairros"]}
    preco_m2 = bairros.get(bairro, 0)
    func_por_m2 = PREMISES["moradia"]["funcionariosPor1000m2"] / 1000.0
    base_func = max(1, math.ceil(metragem * func_por_m2))
    custo_func = PREMISES["moradia"]["custoFuncionario"]
    custo_base_pessoa = PREMISES["moradia"]["custoBasePorPessoa"]

    escolas_map: Dict[str, List[Tuple[int, int, float]]] = {}
    for esc in PREMISES["educacao"]["escolas"]:
        escolas_map.setdefault(esc["nome"], []).append((esc["idadeMin"], esc["idadeMax"], esc["precoAnual"]))
    faculdade_br = PREMISES["educacao"]["faculdade"]["brasil"]
    faculdade_ext = PREMISES["educacao"]["faculdade"]["exteriorUSD"]

    viagem_custos = PREMISES["lifestyle"]["viagensInternacionais"]["custoUSD"]
    estilos = PREMISES["lifestyle"]["estilosDeVida"]
    estilo_cfg = estilos.get(estilo_vida, estilos[1])
    ...
    for ano in anos:
        ...
        custo_veic_brl = total_carros * PREMISES["veiculos"]["gastoAnualPorVeiculo"] * infl_factor_brl + custo_compra_veic
...
def main():
    ...
    if "stage" not in st.session_state:
        st.session_state.stage = "inputs"
    # Data storage
    # Initialise keys in session_state.  We prefer blank or zero defaults so that
    # the UI does not pre‑populate values when first loaded.  The old fields
    # related to renda extra/dividendos foram removidos e novos campos para
    # aluguéis, dividendos e DY foram adicionados.
    for key in [
        "cargo",
        "setor",
        "empresa",
        ...
        "risk_profile",
        "nao_tem_conjuge",
        "salario_manual_input",
        "api_failed",
        "warning_no_salary",
        "aspirational_inicial",
        "aspirational_growth_rate",
    ]:
    ...
    if st.session_state.stage == "inputs":
        ...
        if n > 0:
            st.subheader("Filhos")
            school_names = sorted({esc["nome"] for esc in PREMISES["educacao"]["escolas"]})
            for i in range(n):
        ...
        bairro_options = ["Selecione"] + [b["nome"] for b in PREMISES["moradia"]["bairros"]]
        bairro_index = 0
    ...
    # Stage: Results and projections
        ...
        with tab_dados:
            ...
            col_r1, col_r2 = st.columns(2)
            bairro_opts = ["Selecione"] + [b["nome"] for b in PREMISES["moradia"]["bairros"]]
            bairro_idx = 0
